...
# helper functions
plot.pca.study <- function(study, meta.mat, counts.mat, cols=cols.dates) {
  m <- filter(meta.mat, Study==study)
  pca <- DESeqDataSetFromMatrix(counts.mat[, rownames(m)], m, ~1) %>%
...
plot.pca.species <- function(species, meta.mat, counts.mat, 
                             cols=cols.dates, shapes=shapes.strain) {
  m <- filter(meta.mat, Species==species)
  pca <- DESeqDataSetFromMatrix(counts.mat[, rownames(m)], m, ~1) %>%
         varianceStabilizingTransformation() %>%
  ...
  pca %>%
    ...
    scale_shape_manual(NULL, values=shapes) +
    scale_fill_manual(NULL, values=cols) +
    labs(x=pcs[1], y=pcs[2],
         title=species) +
    guides(fill=guide_legend(override.aes=list(pch=21, size=5)),
...
plot.celltypes.study <- function(stats.mat, celltypes=cibx) {
  stats.mat %>%
    select(Celltype, Species, Strain) %>%
    distinct() %>%
    left_join(celltypes, by=c("Celltype", "Species", "Strain")) %>%
    ggplot(aes(Daterange, Score)) +
    geom_boxplot(col="black", fill="lightgrey") +
    ggpubr::stat_pvalue_manual(data=stats.mat, color="#e41a1c") +
    scale_y_continuous("Celltype score", expand=expansion(c(0.05, 0.1))) +
...
plot.celltypes.species <- function(stats.mat, celltypes=cibx, 
                                   cols=cols.strain) {
  stats.mat %>%
    select(Celltype, Species) %>%
    distinct() %>%
    left_join(celltypes, by=c("Celltype", "Species")) %>%
    ggplot(aes(Daterange, Score)) +
...
meta <- readxl::read_excel("samplesheet.xlsx", "samples.nanostring") %>%
...
# cyno
qunt <- read.delim("rnaseq/inputs/mfascicularis.tsv.summary") %>%
...
# AGM
qunt <- read.delim("rnaseq/inputs/csabaeus.tsv.summary") %>%
        filter(Status=="Assigned") %>%
        reshape2::melt(id.vars="Status",
                       variable.name="ID",
                       value.name="Reads") %>%
...
## load counts -----------------------------------------------------------------
cmat <- read.delim("rnaseq/inputs/mfascicularis.tsv") %>%
...
cmat %>%
  ...
  as.data.frame() %>%
  rownames_to_column("Gene") %>%
  write.table("rnaseq/inputs/cibersort.tsv", sep="\t")
# once run, load CIBERSORT output
cibx <- read.csv("rnaseq/outputs/cibersort.csv", check.names=FALSE) %>%
        filter(`P-value` < 0.05) %>%
...
supp08$c <- plot.pca.study("AGM, 9530537", meta.mat=meta, counts.mat=cmat)
supp08$d <- plot.pca.study("AGM, Malale", meta.mat=meta, counts.mat=cmat)
supp08$e <- plot.pca.study("Cyno, 9530537", meta.mat=meta, counts.mat=cmat)
supp08$f <- plot.pca.study("Cyno, Malale", meta.mat=meta, counts.mat=cmat)

# what is the strain-specific effect for each species, if any? (combine strains)
supp10$a <- plot.pca.species("AGM", meta.mat=meta, counts.mat=cmat)
supp10$b <- plot.pca.species("Cyno", meta.mat=meta, counts.mat=cmat)
...
rm(de)
write.csv(rmat, "rnaseq/outputs/results-study.csv", row.names=FALSE)
...
rmat %>%
  ...
  write.csv("rnaseq/inputs/ipa-study.csv", row.names=FALSE)

# ALT OPTION: load pre-saved results matrix
rmat <- read.csv("rnaseq/outputs/results-study.csv") %>%
        mutate(Daterange=factor(Daterange, levels=names(cols.dates)))
...
supp08$g <- rmat %>%
            ...
            mutate(DPI=factor(Daterange, 
                              levels=names(dpi.dates), 
                              labels=dpi.dates),
...
subtypes %>%
  ...
  plot.celltypes.study() +
  labs(title="AGM, Malale")
ggsave("rnaseq/plots/cibersort-agm-malale.png",
       units="in", width=7.5, height=3)
subtypes %>%
  filter(Species=="AGM", Strain=="9530537") %>%
  plot.celltypes.study() +
  labs(title="AGM, 9530537")
ggsave("rnaseq/plots/cibersort-agm-9530537.png",
       units="in", width=5, height=3)
subtypes %>%
  filter(Species=="Cyno", Strain=="Malale") %>%
  plot.celltypes.study() +
  labs(title="Cyno, Malale")
ggsave("rnaseq/plots/cibersort-cyno-malale.png",
       units="in", width=7.5, height=6)
subtypes %>%
  filter(Species=="Cyno", Strain=="9530537") %>%
  plot.celltypes.study() +
  labs(title="Cyno, 9530537")
ggsave("rnaseq/plots/cibersort-cyno-9530537.png",
       units="in", width=7.5, height=6)
rm(subtypes)
...
# save results and clean up
rm(de)
write.csv(rmat, "rnaseq/outputs/results-strain.csv", row.names=FALSE)
...
rmat %>%
  ...
  reshape2::dcast(Gene ~ Group, value.var="value") %>%
  write.csv("rnaseq/inputs/ipa-strain.csv", row.names=FALSE)

# ALT OPTION: load pre-saved results matrix
rmat <- read.csv("rnaseq/outputs/results-strain.csv") %>%
        mutate(Daterange=factor(Daterange, levels=names(cols.dates)))
...
subtypes %>%
  filter(Species=="AGM") %>%
  plot.celltypes.species() +
  labs(title="MACV strain differences (AGM)")
ggsave("rnaseq/plots/cibersort-agm.png",
       units="in", width=5, height=3)
subtypes %>%
  filter(Species=="Cyno") %>%
  plot.celltypes.species() +
  labs(title="MACV strain differences (Cyno)")
ggsave("rnaseq/plots/cibersort-cyno.png",
       units="in", width=5, height=6)
# plot AGM plasma cell for supplemental figure
supp10$e <- subtypes %>%
            filter(Species=="AGM", Celltype=="Plasma cells") %>%
            select(Celltype, Species) %>%
            distinct() %>%
            left_join(cibx, by=c("Celltype", "Species")) %>%
            ggplot(aes(Daterange, Score)) +
            geom_boxplot(aes(group=interaction(Daterange, Strain), fill=Strain), 
                         col="black", outlier.shape=21) +
...
supp11$a <- rmat %>%
            right_join(genelist, by=c("Gene", "Strain", "Daterange")) %>%
            reshape2::dcast(Gene + Strain + Daterange + Group ~ Species, 
                            value.var="lfc") %>%
            na.omit() %>%
            ggplot(aes(Cyno, AGM)) +
            ...
            scale_color_manual("Only DE in", values=cols.species, 
                               na.value="lightgrey") +
...
# plot just D14/terminal for the main figure
fig6$c <- rmat %>%
          right_join(genelist) %>%
          reshape2::dcast(Gene + Strain + Daterange + Group ~ Species, 
                          value.var="lfc") %>%
...
supp11$b <- difs %>%
            ...
            geom_line(aes(col=Group, group=interaction(Group, Strain)), 
                      linewidth=1) +
...
## assemble figures ------------------------------------------------------------
# main figure a-c
x <- cowplot::plot_grid(fig6$b, fig6$c, nrow=1, labels=c("b", "c"))
cowplot::plot_grid(fig6$a, x, nrow=2, labels=c("a", NA), rel_heights=c(2.5, 3))
ggsave("figure-main06-abc.png", units="in", width=7.5, height=5.5)
rm(x)

# supplemental figure 8
cowplot::plot_grid(plotlist=supp08, labels="auto", ncol=2)
ggsave("figure-supplementary08.png", units="in", width=7.5, height=10)

# supplemental figure 9
cowplot::plot_grid(plotlist=supp09, labels="auto", nrow=2)
ggsave("figure-supplementary09.png", units="in", width=7.5, height=10)

# supplemental figure 10
x <- cowplot::plot_grid(supp10$a, supp10$b, nrow=1, labels=c("a", "b"))
y <- cowplot::plot_grid(supp10$d, supp10$e, nrow=1, labels=c("d", "e"))
cowplot::plot_grid(x, supp10$c, y, nrow=3, labels=c(NA, "c", NA),
                   rel_heights=c(1, 2, 2))
ggsave("figure-supplementary10.png", units="in", width=7.5, height=10)
rm(x, y)

# supplemental figure 11
cowplot::plot_grid(plotlist=supp11, labels="auto", nrow=2, rel_heights=c(2, 1))
ggsave("figure-supplementary11.png", units="in", width=7.5, height=7.5)
